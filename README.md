...
PolyRace 是一个创新的区块链预测游戏平台，将加密货币市场与AI预测相结合，采用**像素游戏风格**设计。系统选定两种代币作为"龟"（稳健防御）和"兔"（急速增长），基于Polymarket的链上真实数据，由AI实时分析并预测胜率。用户可以下注支持自己看好的代币，在设定时间范围内，根据代币的价格涨跌幅度决定最终赢家。

### 核心特性

- 🎮 **复古像素游戏风格**：80年代街机美学，怀旧赛博朋克设计
- 🐢🐰 **龟兔赛跑机制**：蓝色龟队（稳健防御） vs 绿色兔队（急速增长）
- 🤖 **AI实时预测**：集成Gemini 1.5 Flash AI，基于Polymarket WebSocket实时数据流分析
- 📊 **链上数据验证**：所有预测和结算基于Polymarket CLOB的真实链上数据
- 🎨 **动态赛道系统**：实时可视化展示两种代币的竞争状态，流畅动画
- 💰 **用户跟注系统**：用户下注会影响赔率和AI预测概率
- 🎙️ **AI实况解说**：AI实时生成中文解说，可选语音播报（TTS）
...
- **Turtle Blue (龟队)**: `#2498e4` - 稳健防御
- **Rabbit Green (兔队)**: `#5cce59` - 急速增长
- **Versus Purple (AI)**: `#7c3aed` - AI预测与对决
- **Deep Space (背景)**: `#0f1218` - 深空黑
- **Card Surface**: `#1a1e29` - 卡片表面

### 视觉元素

- **字体**: Press Start 2P (标题) + VT323 (正文/代码) - 经典像素字体
- **边框**: 像素切角 + 3D像素阴影效果
- **按钮**: 像素按钮 + 3D凸起效果，按下时下沉
- **背景**: 像素网格 + 扫描线动画 + 霓虹光效
- **动画**: 像素化闪烁 + 快速切换过渡
...
- **样式**: Tailwind CSS（像素游戏风格主题）
- **字体**: Press Start 2P (像素标题) + VT323 (像素正文)
- **动画**: Framer Motion（像素化过渡 + 快速响应动画）
